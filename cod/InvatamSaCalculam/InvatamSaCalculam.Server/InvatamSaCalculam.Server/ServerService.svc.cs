...
namespace InvatamSaCalculam.Server
{
    ...
    public class ServerService : IServerService
    {
        ...
        {
            return BusinessStructure.Instance.Model.Users.Where(item => item.IsTeacher == false).OrderByDescending(item => item.UserScores.FirstOrDefault().Score + item.UserGames.FirstOrDefault().Blocks + item.UserGames.FirstOrDefault().Hangman).Take(10).ToList();
        }
